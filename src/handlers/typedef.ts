import { userSchema } from "../utils/helpers";
...
function generateInputTypeForArrayOfObjects(obj, typeName) {
  try {
...
function generateObjectType(obj, typeName) {
  try {
    let objectTypeDef = `
    type ${typeName} {
      ${Object.entries(obj)
...
function generateInputType(obj, typeName) {
  try {
    let inputTypeDef = `
    input ${typeName} {
      ${Object.entries(obj)
...
function getGraphQLType(type, fieldName, parentName = "") {
  try {
    switch (typeof type) {
      case "string":
        // Handle base types
